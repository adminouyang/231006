...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(threadName)s - %(message)s', handlers=[logging.FileHandler("py/TV/config/error_urls_blacklist.txt", "w", encoding="utf-8"), logging.StreamHandler()])
...
if __name__ == "__main__":
    with open("py/TV/config/failed_urls.txt", 'r', encoding='utf-8') as f:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers = 512) as executor:
            ...
            with open("py/TV/config/error_urls_host_blacklist.txt", "w", encoding="utf-8") as error_urlsf:
